...
func (s Service) CreatePolicy(ctx context.Context, policy model.Policy) ([]model.Policy, error) {
	...
	if err != nil {
		fmt.Println(fmt.Sprintf("Error while creating policy! %s", err.Error()))
		return []model.Policy{}, err
	}

	schemas, err := s.generateSchema(policies)
	if err != nil {
		fmt.Println(fmt.Sprintf("Error while creating schema! %s", err.Error()))
		return []model.Policy{}, err
	}
	err = s.pushSchema(ctx, schemas)
	if err != nil {
		fmt.Println(fmt.Sprintf("Error while pushing schema! %s", err.Error()))
		return []model.Policy{}, err
	}
	return policies, err
}
